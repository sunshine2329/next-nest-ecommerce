...
export const fetchProducts =
  ...
  async (dispatch: Dispatch<ProductsAction>) => {
    try {
      ...
      const { data } = await proshopAPI.get(`/products?keyword=${keyword}&pageId=${pageId}`)
...
export const fetchTopRatedProducts = () => async (dispatch: Dispatch<ProductsAction>) => {
  try {
    dispatch({
...
export const fetchProduct = (id: string) => async (dispatch: Dispatch<ProductsAction>) => {
  try {
    dispatch({
...
export const fetchProductReset = () => async (dispatch: Dispatch<ProductsAction>) => {
  dispatch({
    type: ActionTypes.FETCH_PRODUCT_RESET
  })
}

export const deleteProduct = (id: string) => async (dispatch: Dispatch<ProductsAction>) => {
  const config = {
...
export const createProduct = () => async (dispatch: Dispatch<ProductsAction>) => {
...
export const createProductReview =
  (id: string, review: Review) => async (dispatch: Dispatch<ProductsAction>) => {
    const config = {
      withCredentials: true
    }
    ...
    try {
      ...
      const { data } = await proshopAPI.put(`/products/${id}/review`, review, config)
