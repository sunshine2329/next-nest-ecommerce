...
export const createOrder = (order: OrderInterface) => async (dispatch: Dispatch<OrderAction>) => {
...
export const fetchOrder = (id: string) => async (dispatch: Dispatch<OrderAction>) => {
  const config = {
    headers: {
      'Content-Type': 'application/json'
    },
...
export const payOrder =
  (orderId: string, paymentResult: PaymentResult) => async (dispatch: Dispatch<OrderAction>) => {
    const config = {
      headers: {
        'Content-Type': 'application/json'
      },
...
export const fetchUserOrders = () => async (dispatch: Dispatch<OrderAction>) => {
  const config = {
    headers: {
      'Content-Type': 'application/json'
    },
...
export const deliverOrder = (orderId: string) => async (dispatch: Dispatch<OrderAction>) => {
  const config = {
    headers: {
      'Content-Type': 'application/json'
    },
  ...
  try {
    ...
    const { data } = await proshopAPI.put(`/orders/${orderId}/deliver`, {}, config)
